...
from embedding_integrations import run_oct_embedding
...
def test_run_oct_embedding_success(mock_exists, mock_tempfile, mock_run):
    """
    Tests `run_oct_embedding_success` behavior.
    ...
    mock_source_file.name = "source.edges"
    mock_target_file = MagicMock()
    mock_target_file.name = "target.edges"
    
    mock_tempfile.side_effect = [mock_source_file, mock_target_file]
    
    mock_process = MagicMock()
    mock_process.stdout = "v0 -> [1]\nv1 -> [2]"
    mock_process.returncode = 0
    mock_run.return_value = mock_process
    
    result = run_oct_embedding(source, target)
    
    assert result is not None

@patch('embedding_integrations.os.path.exists')
def test_run_oct_embedding_no_exe(mock_exists):
    """
    Tests `run_oct_embedding_no_exe` behavior.
