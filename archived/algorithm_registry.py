...
class AtomAlgorithm(EmbeddingAlgorithm):
    """ATOM — grows its own Chimera topology dynamically. Returns timing only (no chain output)."""
    ...
    def embed(self, source_graph, target_graph, timeout=60.0, **kwargs):
        try:
            ...
            try:
                ...
                # ATOM writes stats to Results.txt, not the chain mapping
                ...
                return {
                    ...
                    'time': elapsed,
                    'method': 'ATOM',
                    'note': 'placeholder — ATOM does not output chain mapping'
                }
